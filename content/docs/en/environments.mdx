...
- **Network policy**: Full internet access or Limited access with a domain allowlist.
...
Limited access uses a domain allowlist for the Session-owned sandbox. The policy is applied before execution and stays fixed for that Session. Create a new Session to use a different policy.
