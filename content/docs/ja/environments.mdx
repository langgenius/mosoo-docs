...
- インターネットアクセスを許可する Full、または 接続先をドメイン許可リストで制限する Limited を選ぶ **Network policy**。
...
Limited は Session 単位の sandbox にドメイン許可リストを適用します。ポリシーは実行前に設定し、その Session 中は固定します。別のポリシーを使うには新しい Session を作成します。
