...
选择用于 packages、setup script、variables 和网络策略的 runtime template。Agent 未显式选择时使用 Project default。新 session 会冻结启动时选中的 Environment revision。Task Agent 支持 Full 或 Limited 网络访问；Assistant Agent 必须使用 Full。详见 [Environment 网络策略](/docs/zh-Hans/environments#网络策略)。
