---
title: "常见 Agent 模式"
description: "为常见工作流选择 Session、预设与 mosoo 功能。"
---
...
- **原因：**同一个 Session 的后续轮次复用工作目录，能帮助长期 copilot。
- **配置：**产品与运营 Skills，以及已授权 MCP connections。
- **边界：**mosoo 当前控制台是单所有者模式，不支持团队角色和共享管理。应通过适当外部界面交付 Agent，而不是共享 owner credential。

## 公共应用

由你的 backend 使用 Project key，为每位应用用户创建或续接 Session。直接传入 harness 配置，或复用可选的 Agent 预设；已有 v1 集成可以继续使用已发布 Agent 的流程。
