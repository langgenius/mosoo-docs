...
test('worker asset preparation localizes static translated docs HTML', async () => {
  ...
  await mkdir(join(out, 'docs', 'ja', 'quickstart'), { recursive: true });
  ...
  await writeFile(
    join(out, 'docs', 'ja', 'quickstart', 'index.html'),
    '<!doctype html><html lang="en"><body>クイックスタート</body></html>',
  );
  ...
  assert.match(
    await readFile(join(out, 'docs', 'ja', 'quickstart', 'index.html'), 'utf8'),
    /<html lang="ja"/,
  );
});
